...
gboolean state_load(gchar* filename, params* p)
{
    ...
    if (!g_file_test(f_status, G_FILE_TEST_EXISTS | G_FILE_TEST_IS_REGULAR))
    {
        g_free(f_status);

        /* resume file isn't exists... */
        return FALSE;
    }
    ...
    if (!g_key_file_load_from_file(
        file, f_status, G_KEY_FILE_NONE | G_KEY_FILE_KEEP_COMMENTS, NULL
    ))
    {
    ...
    if (g_key_file_has_key(file, "files", "dictionary", NULL))
    {
        ...
        p->file_dict = g_key_file_get_string(file, "files", "dictionary", NULL);
    }
    ...
    if (g_key_file_has_key(file, "cracking", "charset", NULL))
    {
        ...
        p->charset = g_key_file_get_string(file, "cracking", "charset", NULL);
    }
    ...
    if (g_key_file_has_key(file, "cracking", "current", NULL))
    {
        ...
        p->current = g_key_file_get_string(file, "cracking", "current", NULL);
    }
    /* threads=n */
    if ((p->threads < 1) && (g_key_file_has_key(file, "cracking", "threads", NULL)))
    {
        p->threads = g_key_file_get_integer(
            file, "cracking", "threads", NULL
        );
    }
